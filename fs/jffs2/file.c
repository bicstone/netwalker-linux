...
/*
 * file information/data flush to media
 */
static int jffs2_flush(struct file * filp, fl_owner_t id)
{
	int ret = 0;
	if ((filp->f_flags & O_ACCMODE) != O_RDONLY) {
		ret = jffs2_fsync (filp, filp->f_path.dentry, 0);
		if (ret == 0) {
			/*
			 * final data flash to media
			 */
