...
static void esdhc_cd_callback(struct work_struct *work)
{
	...
	if (cd_status && host->flags & SDHCI_CD_PRESENT) {
		host->flags &= ~SDHCI_CD_PRESENT;
		changed = 1;
	}
	else if (!cd_status && !(host->flags & SDHCI_CD_PRESENT)) {
		host->flags |= SDHCI_CD_PRESENT;
		changed = 1;
	}
	else
	...
	if (changed && !(host->flags & SDHCI_CD_PRESENT)) {
		printk(KERN_INFO
...
void esdhc_foce_cd_local(int id)
{
	unsigned long flags;
	unsigned int cd_status = 0;
...
EXPORT_SYMBOL(esdhc_foce_cd_local);
...
static int sdhci_resume(struct platform_device *pdev)
{
	...
	for (i = 0; i < chip->num_slots; i++) {
		...
		cd_status = host->plat_data->status(host->mmc->parent);
		if (( cd_status &&  (host->flags & SDHCI_CD_PRESENT)) ||
		    (!cd_status && !(host->flags & SDHCI_CD_PRESENT))) {
			schedule_work(&host->cd_wq);
		} else if (!cd_status &&  (host->flags & SDHCI_CD_PRESENT)) {
			sdhci_init(host);
			mmiowb();
