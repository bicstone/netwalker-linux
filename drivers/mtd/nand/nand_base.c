			     ...
			     struct mtd_oob_ops *ops)
{
	...
	if ((unsigned long)to <= (unsigned long)(chip->pagebuf << chip->page_shift) &&
	    (unsigned long)(chip->pagebuf << chip->page_shift) < (unsigned long)(to + ops->len))
#else
						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
	...
	if (!type->pagesize) {
...
#ifdef CONFIG_MACH_MX51_ERDOS
	    if ((*maf_id == 0xec) && (dev_id == 0xd5) &&
	        (chip->cellinfo == 0x94) && (extid == 0x29)) {
		/*
		 * K9GAG08U0D
		 */
		/* Calc pagesize */
		...
		mtd->erasesize = (128 * 1024) << ((extid & 0x30) >> 4);
