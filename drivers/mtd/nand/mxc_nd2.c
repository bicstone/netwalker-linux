...
static void auto_cmd_interleave(struct mtd_info *mtd, u16 cmd)
{
	...
	case NAND_CMD_READSTART:
		for (i = 0; i < j; i++) {
			...
			/*
			 * erase bad mark for next write
			 */
			((u16 *)(SPARE_AREA0))[0] = 0xffff;
			((u16 *)(SPARE_AREA0))[1] = 0xffff;
#endif
			/* update the value */
			dbuf += dlen;
			obuf += olen;
...
#if 0
	/*
	 * ECC correctable information
	 */
	if (0 < ret) {
		u32 alow  = raw_read(NFC_FLASH_ADDR0);
		u32 ahigh = raw_read(NFC_FLASH_ADDR8);
		printk("Correctable RS-ECC Error +%d(%d) Symbol %x\n", ret, mtd->ecc_stats.corrected, (alow >> 16 | ahigh << 16) * mtd->writesize);
	}
#endif
...
#else
 		/**
 		 * not used last eraseblock(0x80000000 + 0x80000000 = 0x00000000)
 		 */
		mtd->size = (this->chipsize - mtd->erasesize) * num_of_interleave;
#endif
