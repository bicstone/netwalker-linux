...
static phy_info_t phy_info_lan8700 = {
	...
	(const phy_cmd_t []) { /* config */
		...
		{ mk_mii_read(MII_REG_ANAR), mii_parse_anar },
#if 1 //GGGGG
		{ mk_mii_write(MII_REG_CR, 0x8000), NULL }, /* PHY reset */
#endif
		{ mk_mii_end, }
	},
	(const phy_cmd_t []) { /* startup */
...
#if 1 //GGGGGG
	/*
	 * fixed rate from redboot
	 */
	fep->phy_speed =
		((((rate + 499999) / 2500000) / 2) & 0x3F) << 1;
#else
	fep->phy_speed =
 ...
 */
static int  phy_aneg_complete;
static void phy_aneg_comp(uint mii_reg, struct net_device *dev)
{
	struct fec_enet_private *fep = netdev_priv(dev);
	uint status;
...
static phy_cmd_t const phy_cmd_aneg_comp[] = {
	{ mk_mii_read(MII_REG_SR), phy_aneg_comp },
	{ mk_mii_end, }
	};
#endif
...
fec_enet_open(struct net_device *dev)
{
...
#if 1 //GGGGG
	{
		...
		phy_aneg_complete = 0;
		for ( i = 0 ; i < 10 ; i++ ) {
			mii_do_cmd(dev, phy_cmd_aneg_comp);
			ssleep(1);
			if ( phy_aneg_complete ) {
				break;
			}
		}
		if (1000 <= i) {
			printk ("fec_enet_open: AutoNegotiation failed %d\n", i);
		}
	}
#endif
