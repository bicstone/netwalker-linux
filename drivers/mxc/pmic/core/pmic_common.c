 ...
 * ------------------------
 * 2009/08/10 : check while any interrupt.
 *
 */
...
static volatile int pmic_bh_handler_statistics [2];
...
void pmic_bh_handler(struct work_struct *work)
{
	...
	pmic_bh_handler_statistics [0]++;
	if (2 <= retry) {
		pmic_bh_handler_statistics [1]++;
	}
#else
	count = pmic_get_active_events(active_events);
	pr_debug("active events number %d\n", count);
