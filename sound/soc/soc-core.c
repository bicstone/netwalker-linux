...
static int soc_codec_close(struct snd_pcm_substream *substream)
{
	 ...
	 */
	if (codec->active == 0 && substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		snd_soc_dai_digital_mute(codec_dai, 1);

	if (codec->active == 0 && cpu_dai->ops.shutdown)
		cpu_dai->ops.shutdown(substream);

	if (codec->active == 0 && codec_dai->ops.shutdown)
		codec_dai->ops.shutdown(substream);

	if (codec->active == 0 && machine->ops && machine->ops->shutdown)
		machine->ops->shutdown(substream);

	if (codec->active == 0 && platform->pcm_ops->close)
		platform->pcm_ops->close(substream);
