...
static void headphone_detect_handler(struct work_struct *work)
{
	...
	hp_status = plat->amp_enable(-1);
	if (hp_status == 0){
		set_irq_type(plat->hp_irq, IRQ_TYPE_LEVEL_LOW);
	} else {
		set_irq_type(plat->hp_irq, IRQ_TYPE_LEVEL_HIGH);
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	struct platform_device *pdev = priv->pdev;
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	enum snd_soc_bias_level bias_level = SND_SOC_BIAS_OFF;
	...
	if(plat->hp_status != NULL){
		if(plat->hp_status() != 0){
			bias_level = codec->bias_level;
		}
	}
	if (bias_level == SND_SOC_BIAS_ON){
		codec->set_bias_level(codec, SND_SOC_BIAS_PREPARE);
	}
	snd_soc_dapm_sync(codec);
	if (bias_level == SND_SOC_BIAS_ON){
		codec->set_bias_level(codec, SND_SOC_BIAS_ON);
	}
	return 1;
}
...
static int __devinit imx_3stack_sgtl5000_probe(struct platform_device *pdev)
{
	...
	if (plat->hp_status() == 0){
		ret = request_irq(plat->hp_irq,
				  ...
				  IRQ_TYPE_LEVEL_LOW, pdev->name, priv);
	} else {
		ret = request_irq(plat->hp_irq,
				  imx_headphone_detect_handler,
				  IRQ_TYPE_LEVEL_HIGH, pdev->name, priv);
	}
	if (ret < 0) {
